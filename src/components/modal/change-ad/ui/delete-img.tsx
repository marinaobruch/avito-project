...
export const ModalImgDelete: FC<IProps> = ({
  setOpenModalDelete,
  adId,
  file_url,
}) => {
  ...
  const handleDeleteAd = (e: React.MouseEvent<Element, MouseEvent>) => {
    e.stopPropagation()
    deleteImgAd({ id: adId, file_url: file_url }).then((res) =>
      console.log(res),
    )
  ...
  return (
    <div
      ...
      className='w-full h-full fixed left-0 top-0 bg-gray-800/75 z-10 w- flex flex-col items-center justify-center'
    >
      <div
        className='w-96 h-60 bg-slate-200 rounded-2xl flex-col w- items-center justify-evenly'
