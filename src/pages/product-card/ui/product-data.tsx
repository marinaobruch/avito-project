...
export const ProductAdData: FC<IProps> = ({ adById }) => {
	...
	const phoneNumber: string | undefined = adById?.user?.phone
		? adById?.user?.phone
		: 'нет номера'
	const phoneNumberHide = adById?.user?.phone
		? CreateHideNumber(phoneNumber)
		: 'нет номера'
	...
	return (
		<div className='flex flex-col gap-2'>
			<div>
				<div className='text-3xl pb-3 max-w-xl font-robotoMedium'>
					{adById?.title}
				</div>
			...
			<div className='text-3xl pt-9 pb-5 lg:py-5 font-robotoMedium'>
				{numberWithSpaces(adById?.price)} ₽
			</div>
			{model === 'user' ? (
				<div>
					<ButtonMain
			...
			{openModalRedactor && (
				<ChangeAd setOpenModalRedactor={setOpenModalRedactor} adById={adById} />
			)}
			{openModalDelete && (
				<ModalCardDelete setOpenModalDelete={setOpenModalDelete} adId={adId} />
			)}
			{openModalComments && (
				<Comments
